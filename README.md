...
| 6 | `pitch` | 見下ろし角。度、正で見下ろし。Desktopではそのまま、VRでは立ち位置の決定にだけ使う |
...
**Desktopで指定したのと同じ場所・同じ水平向き**からVRが始まります（見下ろし角が急なときの
扱いは[見下ろしの強い視点](#見下ろしの強い視点)を参照）。
...
pitchを再現できないぶん、**見下ろし角が急な視点はそのままの立ち位置だと的を外します**。
Desktopで真上から覗き込んでいた視点にそのまま立つと、正面には被写体の上——たいてい空——が
来てしまいます。被写体中心のKISS-GS空間で顕著です。

そこで立ち位置だけを補正します。`view=` の6値から注視点（`orbitTargetOf`）を割り出し、
**注視点も半径 `distance` も水平向き `yaw` も変えずに**、orbitを見下ろし角の上限
（`app/view-pose.ts` の `XR_SPAWN_PITCH_LIMIT`、20°）まで起こした位置へ立たせます
（`xrSpawnPose`）。被写体は同じ距離・同じ見かけの大きさのまま、正面からわずかに下——
自然に見下ろせるところ——に来ます。

水平まで完全に起こさないのは、そこまで下げると被写体と目線が同じ高さになり、手前の地面や
草に埋もれるためです。**上限内の視点は1mmも動かしません**（`pitch === clamp(pitch)` の
ときはDesktopのeyeをそのまま返します）。部屋のキャプチャのようにほぼ水平な既存リンクの
立ち位置は変わりません。

`view=` のフォーマットは変えていません。注視点は6つの数値から計算できるので、
既に配ってあるリンクもそのまま新しい規則で開けます。
